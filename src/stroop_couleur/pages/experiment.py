...
dash.register_page(__name__, path="/experiment", title="Expérience – Stroop classique")
...
def handle_event(color1, color2, color3, color4, current_res, trial, start_time):
    ...
    if trigger == "new-word":
        ...
        trial_data = (
            {
                ...
                "congruent": displayed_color == COLORS[displayed_word],
                "nb_trial": (trial[0]["nb_trial"] + 1) if trial else 1,
            },
        )
        ...
        return (
            displayed_word,
            {"color": displayed_color, "fontSize": "48px"},
            trial_data,
            start_time,
        )
        ...
        with open("./results.txt", "a") as fp:
            fp.write(
                f"Temps de réponse;{rt};correct;{correct};displayed_word;{trial[0]['displayed_word']};displayed_color;{displayed_color};congruence;{trial[0]['congruent']};\n"
            )
