...
style = {"fontSize": "64px", "color": "gray", "textAlign": "left"}
        ...
        children=[
            ...
            html.Div(
                ...
                children=dbc.Row(
                    [
                        dbc.Col(
                            html.Div(id="Gauche", children="", style=style), width=4
                        ),
                        dbc.Col(
                            html.Div(
                                id="Center",
                                children="Appuyez sur la barre d'espace pour poursuivre",
                                style=style,
                            ),
                            width=4,
                        ),
                        dbc.Col(
                            html.Div(id="Droite", children="", style=style), width=4
                        ),
                    ]
                ),
            ),
...
def handle_keypress(value, trial, start_time):
    ...
    if key == " ":
        ...
        machin = (
            ...
            num if position == "right" else "",
            {
                ...
                "repeat": repeat,
                "congruent": congruent,
                "nb_trial": (trial["nb_trial"] + 1) if trial else 1,
            },
            time.time(),
            "",
            " ",
    ...
    if trial and start_time and key in [f"{i}" for i in range(1, 10)]:
        ...
        with open("./results.txt", "a") as fp:
            fp.write(
                f"Temps de réponse;{rt};response;{response};correct;{correct};digit;{trial['digit']};repeat;{trial['repeat']};congruence;{trial['congruent']};\n"
            )
